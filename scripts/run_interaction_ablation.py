...
from src.mlops import log_artifact, log_artifacts_in_dir, log_params_from_dict, start_run

CONFIG_PATH = resolve_config_path(PROJECT_ROOT / "configs" / "config.yaml")
...
def main():
    ...
    run_name = f"interaction_ablation_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"

    with start_run(run_name=run_name, tags={"entrypoint": "run_interaction_ablation"}):
        log_params_from_dict({"seeds": seeds, "modes": modes}, prefix="ablation")
        log_artifact(CONFIG_PATH, artifact_path="config")
        ...
        try:
            for mode in modes:
                for seed in seeds:
                    ...
                    print(f"\n=== Running ablation: mode={mode} seed={seed} ===\n")
                    ...
                    if result.returncode != 0:
                        raise RuntimeError(
                            f"Ablation run failed for mode={mode}, seed={seed}. See log: {log_path}"
                        )
            ...
            log_artifacts_in_dir(ABLATION_DIR, artifact_path="ablation")
        finally:
